...
This is the **Ultralytics YOLO11s** object detection model exported to **OpenVINO™ IR** (Intermediate Representation) format with **INT8** quantization.

The model files in this directory:

* `yolo11s.xml` – network topology
* `yolo11s.bin` – weights
...
# 1. Load model to NPU
model = Model.create_model("yolo11s.xml", device="NPU")
