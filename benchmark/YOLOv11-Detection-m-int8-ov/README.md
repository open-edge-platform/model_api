...
This is the **Ultralytics YOLO11m** object detection model exported to **OpenVINO™ IR** (Intermediate Representation) format with **INT8** quantization.

The model files in this directory:

* `yolo11m.xml` – network topology
* `yolo11m.bin` – weights
...
# 1. Load model to NPU
model = Model.create_model("yolo11m.xml", device="NPU")
