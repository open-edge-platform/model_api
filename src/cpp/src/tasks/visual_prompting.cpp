...
void MaskPredictor::infer(std::vector<cv::Point> positive, std::vector<cv::Point> negative) {
    ...
    for (size_t i = 0; i < positive.size(); i++) {
        point_coord_data.push_back(positive[i].x);
        point_coord_data.push_back(positive[i].y);
        point_label_data.push_back(1);
    }

    tensors[point_coords_tensor_name] = ov::Tensor(ov::element::f32, ov::Shape({positive.size(), 1, 1}), point_coord_data.data());
    ...
    std::cout << predictions.get_shape() << std::endl;
}

VisualPrompting VisualPrompting::create_model(const std::string& encoder_model_path,
    ...
    encoder_adapter->applyModelTransform(VisualPrompting::serialize);
    ...
    return VisualPrompting(encoder_adapter, predictor_adapter, user_config);
}

void VisualPrompting::serialize(std::shared_ptr<ov::Model>& ov_model) {
...
MaskPredictor  VisualPrompting::infer(cv::Mat image) {
    return pipeline.infer(image);
}

std::map<std::string, ov::Tensor> VisualPrompting::preprocess(cv::Mat image) {
    std::map<std::string, ov::Tensor> input = {};
    input.emplace(encoder_adapter->getInputNames()[0], utils::wrapMat2Tensor(image));
    return input;
}

MaskPredictor VisualPrompting::postprocess(InferenceResult& infResult) {
    auto tensorName = encoder_adapter->getOutputNames().front();

    return MaskPredictor(predictor_adapter, std::move(infResult.data[tensorName]), infResult.inputImageSize);
}
